	...
	public function process()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$data=array(
				...
				'sub_prerent_cata_id'=>$this->input->post('subp_id', true),
				...
				// 'meta_description'=>$this->input->post('meta_description', true)
					...
					//    $sale_price=$this->input->post('sale_price', true);
					//    $offer_price=$this->input->post('offer_price', true);
					//    $saleCommission=($sale_price*2)/100;
					//    $offerCommission=($offer_price*2)/100;
					//    if($this->input->post('sp_commission', true)==1){
					// 	$data['sale_price']=$sale_price;
					// 	$data['sale_commission_amount']=$saleCommission;
					// 	$data['sp_commission']=1;
					//    }else{
					// 	$data['sale_price']=$sale_price + $saleCommission;
					// 	$data['sale_commission_amount']=$saleCommission;
					// 	$data['sp_commission']=0;
					//    }

					//    if($this->input->post('op_commission', true)==1){
					// 	$data['offer_price']=$offer_price;
					// 	$data['offer_commission_amount']=$offerCommission;
					// 	$data['op_commission']=1;
					//    }else{
					// 	$data['offer_price']=$offer_price + $offerCommission;
					// 	$data['offer_commission_amount']=$offerCommission;
					// 	$data['op_commission']=0;
					//    }
...
public function price_update_process()
{
	...
	public function edit()
	{
		...
		$data['currencies']=$this->select->select_single_data('currencies','is_visible',1);
		$productArray=$this->select->select_single_data($this->table_name,'id',$id);
		$data['subparentcategories']=$this->select->select_double_data('categories','parent_id',$productArray[0]->category_id,'sub_parent_id',0);
		$data['subcategories']=$this->select->select_single_data('categories','sub_parent_id',$productArray[0]->sub_prerent_cata_id);
...
public function specifications()
    ...
    public function get_sub_p_category(){
		//$consumer_type = $this->input->post('consumer_type');
		$conditions['tblName'] = 'categories';
		$conditions['where'] = array('parent_id'=>$this->input->post('catId'),'sub_parent_id'=>0);
		$conditions['is_visible'] = 1;
		$result = $this->select->select->getResult($conditions);
		echo json_encode($result);
	}
    
    public function get_sub_categories_by_id_p(){
		//$consumer_type = $this->input->post('consumer_type');
		$conditions['tblName'] = 'categories';
		$conditions['where'] = array('sub_parent_id'=>$this->input->post('catId'));
		$conditions['is_visible'] = 1;
		$result = $this->select->select->getResult($conditions);
		echo json_encode($result);
	}
