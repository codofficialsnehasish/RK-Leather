...
if (!function_exists('count_home_categories')) {
    function count_home_categories()
    {
        $ci =& get_instance();
        $ci->db->where('is_popular', 1);
...
if (!function_exists('price_formatted')) {
    ...
    if($discount_rate>0){
        ...
        $priceHtml.='<s id="ComparePrice-product-template"><span class="money">'.select_value_by_id('currencies','id',$currency,'hex').number_format($price,2).'</span></s><span class="product-price__price product-price__price-product-template product-price__sale product-price__sale--single">';
        // $priceHtml.='<s id="ComparePrice-product-template"><span class="money">'.select_value_by_id('currencies','id',$currency,'hex').number_format($original_price,2).'</span></s>';
        $priceHtml.='<span id="ProductPrice-product-template"><span class="money">'.select_value_by_id('currencies','id',$currency,'hex').' '.number_format($price,2).'</span></span></span>';
