<style type="text/css"> 
         ...
         .cartload{margin:40px 0 80px}
         .totalcard{padding:9px 0px}
         .totalcard p{padding-left:9px}
         .totalcard p.right{padding-left:0px;padding-right:9px;font-size:19px}
         .name{margin-bottom: 3px;} 
         .name a{color:#333;color: #3d4241;font-weight: 500;} 
         ...
         .right svg{height: 12px;width: 16px;position: relative;top: -1px;fill: #585252;}
         ...
         .payoption li img{max-height:30px;margin-bottom:10px;}
         ...
         .calbkpan button{position:absolute;right:0;top:0;background:#1b3e5b;outline:none;border:none;color:#fff;height:42px;width:40px;padding:0;border-radius:0px;}
         .calbkpan button .fa{font-size:22px}
         .alert{padding:4px 10px;margin-top:4px}
         .couponbox{margin-bottom:15px}
         .form-inline{display:inline-block!important;}
         .form-inline .form-group{display: inline-block!important;}
         .distxt{color:green;font-weight:600}
         .qtygroup{display:flex;position:relative;white-space:nowrap;vertical-align:top;justify-content: flex-start;}
         .qtygroup input{box-shadow:none;border-radius:0}
         .qtygroup .qtytxt{width:35px;height:30px;padding:0 5px;text-align:center;border:0;margin-left:-4px;margin-right:-4px;font-size:12px;font-weight:500;background:transparent;}
         .qtygroup .minusbtn,.qtygroup .plusbtn{padding:0;height:30px;width:30px;background-color: #02845d;border:0;color:#fff;cursor:pointer;border-radius:50%}
         .qtygroup .minusbtn:hover,.qtygroup .plusbtn:hover{background:#f55d2c;color:#fff!important;transition:.2s;background-image:-webkit-linear-gradient(left,rgba(230,92,91,0.9),rgba(245,93,44,0.9));background-image:linear-gradient(to right,rgba(230,92,91,0.9),rgba(245,93,44,0.9))}
         .qtygroup input::-webkit-outer-spin-button,.qtygroup input::-webkit-inner-spin-button{-webkit-appearance:none;-moz-appearance:none;margin:0}
         .qtygroup .minusbtn:focus,.qtygroup .plusbtn:focus{outline:none}
...
<div class="section">
    <div class="container"></div>
         <div class="cload">
            <center><img src="<?= base_url('assets/static/loading.gif');?>"></center>
         </div>
         <div class="cartload">
			<div class="container">
				<div class="row">
					<div class="col-sm-8 leftpan">
						<div class="card">
							...
							<div class="card-title">My Cart(<?= !empty($cartitems)?count($cartitems):0;?>)</div>
							</div>
							...
							<div class="eachitem">
								<div class="row">
									<div class="col-4 col-sm-2">
										<a href="<?= base_url('product/'.$product->slug);?>"><img src="<?= get_product_main_image($product);?>" class="img-fluid"></a>
									</div>
									<div class="col-8 col-sm-6 propan">
										<p class="name"><a href="<?= base_url('product/'.$product->slug);?>"><?= $product->title;?></a></p>
										...
										?>
										<p>
										<span class="offprice"><?= select_value_by_id('currencies','id',$product->currency_code,'hex');?> <?= $object->price_calculated;?></span>
										<?php if($product->no_discount!=1){?>
										<small class="mrpprice"><strike><?= select_value_by_id('currencies','id',$product->currency_code,'hex');?> <?= $product->price;?></strike></small> 
										<span class="percentage">(<?= $product->discount_rate;?>% Off)</span>
										<?php }?>
										</p>
									</div>
									<div class="col-8 col-sm-3 clearxs">
										<p class="subtotal">Total: <?= $object->price_calculated;?> x <?= $item->quantity;?> = <strong> <?= $object->price_calculated*$item->quantity;?></strong></p>
										<div class="qty qty-changer">
										<div class="qtygroup">
											<input type="button" value="-" class="qtyupdate minusbtn" data-action="1" data-id="1" data-cart-item-id="<?= $item->id;?>" data-product-id="<?= $item->product_id;?>">  
											<input type="number" step="1" name="product_quantity"  data-cart-item-id="<?= $item->id;?>" data-product-id="<?= $item->product_id;?>" id="cart<?= $item->id;?>" value="<?= $item->quantity;?>" class="qtytxt">
											<input type="button" value="+" class="qtyupdate plusbtn" data-action="2" data-id="1" data-cart-item-id="<?= $item->id;?>" data-product-id="<?= $item->product_id;?>">
										</div>
										</div>
									</div>
									<div class="col-4 col-sm-1">
										<a href="javascript:void(0);" class="delbtn" title="Remove From Cart" data-id="1"  onclick="remove_from_cart('<?= $item->id; ?>','cart');">
										<svg class="svg_icon">
										<i class="fa fa-trash text-danger"></i>
											<!-- <use xlink:href="<?= base_url('assets/static/icons.svg#iw-delete');?>"></use> -->
										</svg>
							...
							<?php 
								...
								$currency=select_value_by_id('currencies','id',$product->currency_code,'hex');
									...
									<?php endif;?>
							<div class="orderplc">
								<a href="javascript:;"  class="orderbtn goCheckout">CHECKOUT NOW</a>
							</div>
							</div>
						</div>
					</div>
						...
						<div class="card">
							...
							<div class="row">
								...
								<div class="col-4">
									<p class="right">
										<!-- <svg class="svg_icon">
										<use xlink:href="static/icons.svg#iw-dollar"></use>
										</svg> -->
										<?= formatted_price($cart_total->subtotal, $cart_total->currency);?>
									</p>
									<p class="right">
										<!-- <svg class="svg_icon">
										<use xlink:href="static/icons.svg#iw-dollar"></use>
										</svg> -->
										<?= !empty($totalshippinhg)?formatted_price($totalshippinhg, $cart_total->currency):0;?>
									</p>
								</div>
							</div>
							<div class="hr"></div>
							<p class="payamnt">Amount Payable <span class="pull-right"><?= formatted_price($cart_total->total, $cart_total->currency);?></span></p>
							</div>
						</div>
						<div class="card">
							<div class="card-header">
